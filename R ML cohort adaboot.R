# using RF to classify cancer cohort
...
train.adaboost<-boosting(myFormula, data=RF.trainset, boos=TRUE, mfinal=5)# iterate mfinal times
#with mfinal > 5 does not imply better acc
...
table.trainset.rf<-table(true=RF.trainset$CL, pred=rf.pred)
table.trainset.rf
acc.rf<-signif(sum(diag(table.trainset.rf)) / sum(table.trainset.rf)*100,3)
acc.rf

rf.testset12=abc$CL[test.index_s12]; length(rf.testset12)
rf.testset34=abc$CL[test.index_s34]; length(rf.testset34)
rf.testset=unlist(list(rf.testset12, rf.testset34))
length(rf.testset)
...
table.testset=table(true=rf.testset,pred=test.predict)
table.testset
